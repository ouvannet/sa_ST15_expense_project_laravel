...
<div class="modal-dialog modal-lg"> <!-- Fixed typo: "model-lg" to "modal-lg" -->
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="editExpenseLabel">Edit Expense</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        ...
        <div class="modal-body">
            <form id="editExpenseForm" enctype="multipart/form-data">
                ...
                <div class="row">
                        ...
                        <select class="form-select" id="edit-category" name="category_id" required>
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}"
                                    {{ $expense->categories_id == $category->id ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                            ...
                            @foreach ($users as $user)
                                @if ($user->role_id != 1)
                                    <option value="{{ $user->id }}"
                                        {{ $expense->user_id == $user->id ? 'selected' : '' }}>
                                        {{ $user->name }}
                                    </option>
                                @endif
                            @endforeach
                    ...
                    <div class="col-md-6 mb-3">
                        <label for="edit-budget" class="form-label">Budget</label>
                        <input type="number" class="form-control" id="edit-budget" name="budget"
                            value="{{ $expense->budget }}" required>
                    </div>

                    <div class="col-md-6 mb-3">
                        <label for="edit-balance" class="form-label">Balance</label>
                        <input type="number" class="form-control" id="edit-balance" name="budget_balance"
                            value="{{ $expense->budget_balance }}" readonly>
                    </div>
                        ...
                        <small class="form-text text-muted">
                            ...
                            @if ($expense->attachment)
                                <a href="{{ asset('storage/' . $expense->attachment) }}"
                                    target="_blank">{{ basename($expense->attachment) }}</a>
                            @else
                                None
                            @endif
                        </small>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select" id="edit-status" name="status" required>
                            ...
                            <option value="Canceled" {{ $expense->status == 'Canceled' ? 'selected' : '' }}>Canceled
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select" id="edit-assign" name="assigned_to" required>
                            ...
                            @foreach ($users as $user)
                                @if ($user->role_id == 1)
                                    <option value="{{ $user->id }}"
                                        {{ $expense->assign == $user->id ? 'selected' : '' }}>
                                        {{ $user->name }}
                                    </option>
                                @endif
                            @endforeach
                    ...
                    <div class="text-end">
                        <button type="button" class="btn btn-success" id="btn_submit_edit_expense">Update
                            Expense</button>
                    </div>
                </div>
            </form>
